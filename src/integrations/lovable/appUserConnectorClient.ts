...
export async function connectAppUser(opts: {
  ...
  if (!popup) {
    return { success: false, connectorId, error: "יש לאפשר חלונות קופצים ולנסות שוב" };
  }
  ...
  try {
    ...
    popup.close();
    return { success: false, connectorId, error: e instanceof Error ? e.message : "שגיאה בהתחלת OAuth" };
  }
  ...
  return await new Promise<AppUserOAuthResult>((resolve) => {
    ...
    const onMessage = (event: MessageEvent) => {
      ...
      resolve({ success: false, connectorId, error: data.error ?? "OAuth נכשל" });
    };
    window.addEventListener("message", onMessage);
    const timer = setInterval(() => {
      if (popup.closed) {
        cleanup();
        resolve({ success: false, connectorId, error: "ההתחברות בוטלה" });
      }
    }, 500);
  });
}
